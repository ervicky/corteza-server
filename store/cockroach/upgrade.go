...
func (u upgrader) TableExists(ctx context.Context, table string) (bool, error) {
	var exists bool
	//fmt.Println("____________|||____________> ", table)
	if err := u.s.DB().GetContext(ctx, &exists, fmt.Sprintf("SELECT * FROM public.%s", table)); err != nil {
		// @fixme use regex?! -> ADD IF NOT EXISTS
		if err.Error() == fmt.Sprintf("pq: relation \"public.%s\" does not exist", table) {
			exists = false
		} else {
